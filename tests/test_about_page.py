...
class TestAboutPage:
    def test_about_page(self, action_page, about_page, assertions):
        ...
        count_elements2 = assertions.all_index_locator(about_page.menu_advantages.collections_of_repeating_locators())
        assert (assertions.all_index_locator(about_page.menu_advantages.collections_of_repeating_locators()) ==
                menu_advantage_count),f"Ожидали количество элементов {menu_advantage_count}! Получили:{count_elements2}"
        count_elements3 = assertions.all_index_locator(about_page.collections_of_repeating_locators())
        assert (assertions.all_index_locator(about_page.collections_of_repeating_locators()) ==
                about_count), f"Ожидали количество элементов {menu_news_count}! Получили: {count_elements3}"
